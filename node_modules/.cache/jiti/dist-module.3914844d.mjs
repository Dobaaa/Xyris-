...
var _parse = await jitiImport("@iconify/utils/lib/svg/parse");
var _localPkg = await jitiImport("local-pkg");
var _stdEnv = await jitiImport("std-env");
var _picomatch = _interopRequireDefault(await jitiImport("picomatch"));function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _getRequireWildcardCache(e) {if ("function" != typeof WeakMap) return null;var r = new WeakMap(),t = new WeakMap();return (_getRequireWildcardCache = function (e) {return e ? t : r;})(e);}function _interopRequireWildcard(e, r) {if (!r && e && e.__esModule) return e;if (null === e || "object" != typeof e && "function" != typeof e) return { default: e };var t = _getRequireWildcardCache(r);if (t && t.has(e)) return t.get(e);var n = { __proto__: null },a = Object.defineProperty && Object.getOwnPropertyDescriptor;for (var u in e) if ("default" !== u && {}.hasOwnProperty.call(e, u)) {var i = a ? Object.getOwnPropertyDescriptor(e, u) : null;i && (i.get || i.set) ? Object.defineProperty(n, u, i) : n[u] = e[u];}return n.default = e, t && t.set(e, n), n;}
...
const isFullCollectionExists = (0, _localPkg.isPackageExists)("@iconify/json");
...
async function loadCustomCollection(collection, nuxt) {
  ...
  _kit.logger.success(`Nuxt Icon loaded local collection \`${collection.prefix}\` with ${Object.keys(collection.icons).length} icons`);
  return collection;
}
async function parseCustomCollection(collection, nuxt) {
  const dir = (0, _nodePath.isAbsolute)(collection.dir) ? collection.dir : (0, _nodePath.join)(nuxt.options.rootDir, collection.dir);
  const files = (await (0, _tinyglobby.glob)(["*.svg"], {
    cwd: dir,
    onlyFiles: true,
  ...
  const parsedIcons = await Promise.all(files.map(async (file) => {
    let name = (0, _nodePath.basename)(file, ".svg");
    if (normalizeIconName && !validIconNameRE.test(name)) {
      ...
      if (normalized !== name)
      _kit.logger.warn(`Custom icon \`${name}\` is normalized to \`${normalized}\`, we recommend to change the file name to match the icon name, or pass \`normalizeIconName: false\` to your custom collection options`);
      name = normalized;
    }
    let svg = await _promises.default.readFile((0, _nodePath.join)(dir, file), "utf-8");
    const cleanupIdx = svg.indexOf("<svg");
    if (cleanupIdx > 0)
    svg = svg.slice(cleanupIdx);
    const data = (0, _parse.convertParsedSVG)((0, _parse.parseSVGContent)(svg));
    if (!data) {
      _kit.logger.error(`Nuxt Icon could not parse the SVG content for icon \`${name}\``);
      return [name, {}];
    }
    if (data.top === 0)
    delete data.top;
  ...
  _kit.logger.success(`Nuxt Icon loaded local collection \`${collection.prefix}\` with ${successfulIcons.length} icons`);
  const result = {
    ...collection,
...
async function discoverInstalledCollections() {
  const collections = isFullCollectionExists ? collectionNames : collectionNames.filter((collection) => (0, _localPkg.isPackageExists)("@iconify-json/" + collection));
  if (isFullCollectionExists)
  _kit.logger.success(`Nuxt Icon discovered local-installed ${collections.length} collections (@iconify/json)`);else
  if (collections.length)
  _kit.logger.success(`Nuxt Icon discovered local-installed ${collections.length} collections:`, collections.join(", "));
  if (isFullCollectionExists)
  _kit.logger.warn("Currently all iconify collections are included in the bundle, which might be inefficient, consider explicit name the collections you use in the `icon.serverBundle.collections` option");
  return collections;
}
...
${failed.map((f) => " - " + f).join("\n")}`;
      ...
      if (sizeLimitKb > 0) {
        ...
        if (bundleSizeKb > sizeLimitKb * 0.75) {
          _kit.logger.warn(`Nuxt Icon client bundle size is close to the limit: \`${bundleSizeKb.toFixed(2)}KB\` -> \`${sizeLimitKb}KB\``);
        }
      }
...
class IconUsageScanner {
  ...
  isFileMatch(path) {
    return _picomatch.default.isMatch(path, this.globInclude) && !_picomatch.default.isMatch(path, this.globExclude);
  }
  async scanFiles(nuxt, set = /* @__PURE__ */new Set()) {
    const files = await (0, _tinyglobby.glob)(
      this.globInclude,
      {
    ...
    await Promise.all(
      files.map(async (file) => {
        const code = await _promises.default.readFile(file, "utf-8").catch(() => "");
...
class NuxtIconModuleContext {
  ...
  async loadClientBundleCollections() {
    ...
    const { getIconData } = await Promise.resolve().then(() => jitiImport('@iconify/utils').then((m) => _interopRequireWildcard(m)));
    const { loadCollectionFromFS } = await Promise.resolve().then(() => jitiImport('@iconify/utils/lib/loader/fs').then((m) => _interopRequireWildcard(m)));
    const failed = [];
    let count = 0;
...
const _module = exports.default = (0, _kit.defineNuxtModule)({
  ...
  async setup(options, nuxt) {
    const resolver = (0, _kit.createResolver)("file:///C:/Users/TEMP/nuxt3-ecomerece-starter/node_modules/@nuxt/icon/dist/module.mjs");
